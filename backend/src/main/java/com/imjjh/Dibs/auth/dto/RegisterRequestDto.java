...
public record RegisterRequestDto(
        ...
        @Schema(description = "유저 비밀번호", example = "Test1234!@") @NotBlank(message = ValidationMessage.NOT_BLANK) @Pattern(regexp = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]+$", message = ValidationMessage.Auth.PASSWORD_FORMAT) @Size(min = 8, max = 20, message = ValidationMessage.Auth.PASSWORD_SIZE) String password,
